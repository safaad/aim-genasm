...
# WRAM used memory upper limit is estimated using the bitvectors
memory_upper_limit = 2*(nr_of_wrong_bases + 1) * ((read_length+64)/64) * \
    8 + 80 + ((read_length + 64)/64)*8*5 + 4*read_length + 68
...
memory_upper_limit_mram = NR_TASKLETS*read_length*(nr_of_wrong_bases+1)*4*((read_length+64)/64)*8 + (
    number_reads/args["nr_of_dpus"])*4*read_length + (number_reads/args["nr_of_dpus"])*24
